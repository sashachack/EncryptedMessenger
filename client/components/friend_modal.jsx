...
export default function FriendModal({friendModal, setFriendModal, selectedUserID, setSelectedUserID}, props) {
  ...
  const getFriendOptions = async () => {
    ...
    const getUsers = await axios.post('http://localhost:5001/users/all_users', {first, last})
    let temp_options = []
  ...
  const getFriends = async () => {
    ...
    const current_friends = await axios.post('http://localhost:5001/users/get_friends', {username})
    let temp_friends = []
  ...
  return (
    <div className="backdrop-blur-md inset-0 h-screen absolute w-screen flex justify-center m-auto items-center">
      <div className=" bg-bg3 rounded-lg flex flex-col justify-center items-center">
        <button
            className="text-l font-bold text-white p-2">
              <FontAwesomeIcon className="text-white cursor-pointer fa-2x" icon={faTimes}
        ...
        {!no_new_friends && (
          <div className="mt-10 flex justify-center items-center flex-col w-72 rounded-lg shadow-xl h-auto p-2">
            ...
            <button className="text-xl font-bold text-white p-2 rounded-lg" onClick={submit}>
              Add Friend
            </button>
          </div>
        )}
